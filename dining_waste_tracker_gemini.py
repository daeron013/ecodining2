...
async def analyze_plate_with_gemini(before_img: np.ndarray, after_img: np.ndarray) -> Dict:
    ...
    try:
        ...
        prompt = """You are analyzing food waste from dining plate photos. The FIRST image shows a plate BEFORE eating, 
        and the SECOND image shows the SAME plate AFTER eating.

Your task: Identify each distinct food item and calculate how much of each was LEFT UNEATEN (wasted).
    ...
    "food_items": [
        {
            ...
            "initial_portion": "detailed description with quantity (e.g., '2 strips of bacon', '~3 oz grilled chicken breast', 'half cup of rice')",
            "remaining_portion": "detailed description of what's left (e.g., '0.7 strips with fatty ends', '3 potato cubes', 'small clump on left side')",
            "waste_percentage": <number 0-100 representing ONLY the food LEFT on plate, NOT what was eaten>,
            "estimated_weight_oz": <weight in ounces of the WASTED/REMAINING food only>,
            "category": "entree/side/vegetable/dessert/beverage/bread"
        }
    ],
    "overall_assessment": "Brief summary mentioning which items were finished well vs left behind, eating patterns observed",
    "suggestions": ["Specific, actionable tip based on what was left", "Another practical suggestion"]
}

CRITICAL RULES FOR waste_percentage:
- This number represents ONLY the food LEFT ON THE PLATE after eating (the waste)
- It is NOT the amount consumed
- Examples:
  * Ate everything, clean plate = 0-5% waste
  * Ate most, small amount left = 10-20% waste  
  * Ate half, half remains = 50% waste
  * Barely touched, most left = 80-95% waste

ANALYSIS GUIDELINES:
1. Be specific about food items - identify each distinct item separately (e.g., "Scrambled Eggs" and "Bacon" not just "Breakfast")
2. For initial_portion: estimate realistic serving sizes in ounces, cups, or pieces (e.g., "2 strips", "6 oz", "1 cup")
3. For remaining_portion: describe what's visibly left (e.g., "3 potato cubes", "one strip with fatty ends", "small clump in corner")
4. Compare the visual volume/area of food before vs after to calculate waste_percentage accurately
5. For estimated_weight_oz: estimate only the weight of what's LEFT (wasted), not the original portion
6. Be realistic about typical college dining hall portions (8-12 oz entrees, 4-6 oz sides)

RESPONSE QUALITY:
- Identify 2-5 distinct food items typically (don't over-split or over-lump)
- Be precise with percentages based on visual volume comparison
- In overall_assessment, note eating patterns (e.g., "protein consumed well, vegetables left behind")
- Suggestions should be specific to what was wasted (e.g., if vegetables left: "Try taking a smaller vegetable portion initially")
